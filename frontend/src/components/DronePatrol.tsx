    ...
    return (
        <div className="space-y-4">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-bold flex items-center gap-2">
                                Drone Fleet Command
                                <span className={`text-xs px-2 py-0.5 rounded-full ${playing ? "bg-purple-400/10 text-purple-400" : "bg-white/5 text-white/40"}`}>
                                    {playing ? `● ${drones.filter(d => d.status !== "idle").length} Active` : "○ Standby"}
                                </span>
                            </h2>
                            <p className="text-white/40 text-xs">Multi-drone autonomous patrol & emergency response</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 p-1 rounded-2xl bg-black/40 border border-white/5 backdrop-blur-md shadow-inner">
                        <button onClick={() => {
                            ...
                            className={`px-4 py-2 rounded-xl text-xs font-bold tracking-wide flex items-center gap-2 transition-all duration-300 ${playing
                                ? "bg-amber-500/10 text-amber-400 border border-amber-500/20 hover:bg-amber-500/20 shadow-[0_0_15px_rgba(245,158,11,0.1)]"
                                : "bg-purple-500/10 text-purple-400 border border-purple-500/20 hover:bg-purple-500/20 shadow-[0_0_15px_rgba(168,85,247,0.15)]"
                                }`}>
                        ...
                        <button onClick={handleReset}
                            className="px-3 py-2 rounded-xl text-xs font-semibold text-white/50 hover:text-white hover:bg-white/10 transition-colors flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-4 text-[11px] text-white/25">
                <span className="flex items-center gap-1"><Crosshair className="w-3 h-3 text-purple-400/60" /> {drones.length} drones</span>
                <span>Loop: {LOOP_DURATION / 1000}s</span>
                <span className="flex items-center gap-1"><Zap className="w-3 h-3" /> Scan radius: {SCAN_RADIUS}m</span>
